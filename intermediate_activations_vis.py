...
try:
    with open('bert/tpdn_model_tree_with_activations.json', 'r') as f:
...
for role in unique_roles[:5]:  # Check first 5 roles
...
# Apply PCA to reduce dimensionality to 2D (instead of UMAP)
print("Applying PCA to role embeddings...")
role_pca = PCA(n_components=2, random_state=100)
role_embedding_2d = role_pca.fit_transform(role_embeddings)
...
# Create a dictionary to map roles to colors (first 10 unique roles)
unique_roles = list(set(roles))[:10]  # Limit to first 10 unique roles for clarity
...
def visualize_embeddings_html(embedding_2d, hover_text, color_values, title, filename,
                              color_labels=None, color_discrete_map=None):
    ...
    hover_data = {
        'Instance ID': [id_val.split('_')[-1] if isinstance(id_val, str) else id_val for id_val in instance_ids],
    }
    ...
    for point_idx in range(len(hover_text)):
        fig.add_trace(
            go.Scatter(
                ...
                hoverinfo='text',
                hovertext=f"Text: {hover_text[point_idx]}<br>Category: {color_values[point_idx]}<br>Instance: {instance_ids[point_idx]}",
                showlegend=False
            )
        )
...
# Prepare color maps for roles and special tokens
...
# Visualize role embeddings with hover text showing roles
print("Generating interactive role embeddings visualization...")
visualize_embeddings_html(
    role_embedding_2d,
    roles,  # Hover text shows roles
    roles,  # Color by role
    "PCA Visualization of Tree Role Embeddings",
    "role_embedding_pca_tree.html",
    color_discrete_map=role_color_map
)

# Visualize filler embeddings with hover text showing tokens
...
visualize_embeddings_html(
    ...
    "PCA Visualization of Tree Filler Embeddings",
    "filler_embedding_pca_tree.html",
...
visualize_embeddings_html(
    ...
    "PCA Visualization of Tree Filler Embeddings - Frequency",
    "filler_embeddings_pca_tree_frequency.html"
)
